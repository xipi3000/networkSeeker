...
def createGraph():
    ...
    for router in routersExtIps:
        net.node(router, label="", xlabel=router, fontcolor="#c92f00", fontsize="20", fontname="bold",
                 image="./router.png", width="1.2", height="0.8", fixedsize="true")
            ...
            if (len(intf.pointingIps) < 1):
                ...
                for switch, ips in switches.items():
                    ...
                    if intf.intfIp in ips:
                        print(switch)
                        foundIp = True
                        net.edge(routerId, "S" + str(switch + 1), taillabel=intf.intfIp, xlabel="",
                                 label=speed + " bps",
                                 arrowhead="none")
                ...
                if (not foundIp):
                    ...
                    switchId += 1
                    net.node("S" + str(switchId), label="", xlabel="S" + str(switchId), fontcolor="#c92f00",
                             fontsize="20", fontname="bold", image="./switch.png", width="1.2", height="0.8",
                             fixedsize="true")
...
if __name__ == "__main__":
    ...
    for ip in shortest_paths:
        ...
        for target, intermediate in shortest_paths[ip].items():
            if getRouter(target) == getRouter(ip):
                print("Skipping path to " + target + " since they're interfaces on the same router.")
                continue
